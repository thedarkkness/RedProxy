...
echo "  4) MTProto (Telegram)     [$(m "ready" "готово")]"
echo "  5) VLESS + WS + TLS       [$(m "coming soon" "скоро")]"
echo "  6) VMess                  [$(m "coming soon" "скоро")]"
echo "  7) Trojan                 [$(m "coming soon" "скоро")]"
echo "  8) Hysteria2              [$(m "coming soon" "скоро")]"
echo "  9) TUIC                   [$(m "coming soon" "скоро")]"
echo " 10) WireGuard              [$(m "coming soon" "скоро")]"
read -rp "> " choice
...
case "$choice" in
    ...
    4)
        ...
        if mtproto_installed; then
            info "$(m "MTProto is already installed — adding a new client instead." "MTProto уже установлен — добавляю нового клиента.")"
            mtproto_add_client
        else
            while true; do
                read -rp "$(m "Port [8443]: " "Порт [8443]: ")" port; port=${port:-8443}
                if port_in_use "$port"; then
                    warn "$(m "Port $port is already in use on this server:" "Порт $port уже занят на этом сервере:")"
                    port_owner "$port"
                    warn "$(m "Pick a different port (or stop whatever's using it first)." "Выберите другой порт (или сначала остановите то, что его занимает).")"
            ...
            read -rp "$(m "Domain to masquerade as [www.cloudflare.com]: " "Домен для маскировки [www.cloudflare.com]: ")" domain; domain=${domain:-www.cloudflare.com}
            mtproto_install "$port" "$domain"
        fi
        ;;
    *)
